...
Let us now create snapshots of our staging tables. Snapshot tables are named with the prefix `snapshot_`
followed by the name of the model being snapshotted.
For example, the snapshot table for the `stg_users` table would be named `snapshot_stg_users`.
...
```sql
{% snapshot snapshot_stg_merchants %}
...
```sql
{% snapshot snapshot_stg_promotions %}
